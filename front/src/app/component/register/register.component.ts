  ...
  send(valid:boolean) {
    if (valid) {
      if (this.passwordRegister.password == this.passwordRegister.confirmation) {
        ...
        this.userService.post(new Register(this.user, this.passwordRegister)).then(response => { this.success = "Inscription effectuée, vous allez être redirigé vers le login"; setTimeout(() => this.router.navigateByUrl('login'), 3000); }).catch(error => this.error = error);
      }
      else {
